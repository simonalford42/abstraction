...
import box_world
from box_world import SOLVED_IX
...
def hlc_bfs(s0, controller, solved_threshold=0.5, timeout=float('inf')
    ...
    start_solved_logp = controller.solved_logp(start_tau)[SOLVED_IX]
...
def llc_sampler(s: torch.Tensor, b, controller: HeteroController, env) -> tuple[list[int], torch.Tensor]:
    ...
    while True:
        action_logps, stop_logps = controller.micro_policy(s, b)
...
def llc_plan(s: torch.Tensor, abstract_actions, controller, env) -> tuple[list, bool]:
    all_actions = []
    for b in abstract_actions:
        actions, s, done = llc_sampler(s, b, controller, env)
        all_actions.append(actions)

    return all_actions, done
...
def plan(env, controller, timeout):
    start = time.time()
    obs = env.reset()
    box_world.render_obs(obs, pause=2)
    s = box_world.obs_to_tensor(obs).to(DEVICE)
    n = 10
    hl_plan_gen = hlc_bfs(s, controller, timeout=timeout)
    ...
    while True:
        ...
        actions, done = llc_plan(s, actions, controller, env2)
        if done:
...
if __name__ == '__main__':
    ...
    controller = utils.load_model('models/4c2e4c332c1045ae8603f3e58386f126-epoch-0.pt').control_net

    n = 20
    solve_times = multiple_plan(env, controller, timeout=3, n=n)
