...
class AbstractController(nn.Module):
    ...
    def forward(self, state_embeds):
        ...
        """
        # return F.log_softmax(self.net(state_embeds)[:, :self.n])
        ...
        stop_probs = out[:, -2 * self.b: -1 * self.b]
        start_probs = out[:, -1 * self.b:]
        start_probs = F.log_softmax(start_probs, dim=1)

        assertEqual(stop_probs.shape, (T, self.b))
        assertEqual(start_probs.shape, (T, self.b))
        return action_logits, stop_probs, start_probs
...
class Eq2Net(nn.Module):
    ...
    def forward(self, state_embeds, actions):
        ...
        action_logits, _, _ = self.controller(state_embeds)
        step_logits = torch.stack([action_logits[i, 0, actions[i]] for i in range(len(actions))])
