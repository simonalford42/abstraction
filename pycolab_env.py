...
class PyColabEnv(gym.Env):
    ...
    def _paint_board(self, board):
        ...
        Returns:
            3D np.array (np.uint32) representing the RGB of the observation
                layers.
        """
        obs = np.array([list(row.tobytes().decode('ascii')) for row in board])
        return to_color_obs(obs).astype(np.uint8)
        # board_shape = self._last_observations.board.shape
        # board = np.zeros(list(board_shape) + [3], np.uint32)
        # board_mask = np.zeros(list(board_shape) + [3], bool)
    ...
    def _update_for_game_step(self, observations, reward):
        ...
        self._last_state = self._paint_board(observations.board)
