...
class HierarchicalStateSpaceModel(nn.Module):
    ...
    def forward(self, obs_data_list, seq_size, init_size):
        ...
        for t in range(init_size, init_size + seq_size):
            ...
            post_obs_state = self.post_obs_state(concat(obs_post_fwd_list[t - 1], abs_feat))
    ...
    def jumpy_generation(self, init_data_list, seq_size):
        ...
        for t in range(seq_size):
            ...
            abs_feat = self.abs_feat(concat(abs_belief, abs_state)) # this doesn't matter?
            ...
            obs_feat = self.obs_feat(concat(obs_belief, obs_state)) # doesn't matter?
